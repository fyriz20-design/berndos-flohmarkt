    ...
    <div style={{ maxWidth: '650px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif', color: '#333' }}>
      ...
      <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '20px', border: '1px solid #eee', marginBottom: '30px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
        ...
        {cartItems.length === 0 ? (
          ...
          cartItems.map((item, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '15px 0', borderBottom: '1px solid #f9f9f9', fontSize: '18px' }}>
              <span>{item.quantity || 1}x {item.title}</span>
              <span style={{ fontWeight: 'bold' }}>{(Number(item.price) * (item.quantity || 1)).toFixed(2)} €</span>
            </div>
          ))
        )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '30px' }}>
        ...
        <div 
          ...
          style={methodContainerStyle(paymentMethod === 'BANK_TRANSFER')}
        >
          ...
          {paymentMethod === 'BANK_TRANSFER' && settings && (
            <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#f5f3ff', borderRadius: '10px', border: '1px dashed #8b5cf6', fontSize: '16px', lineHeight: '1.6' }}>
              ...
              <p style={{ marginTop: '10px', fontSize: '14px', fontStyle: 'italic' }}>Bitte manuelle Überweisung mit Verwendungszweck: Name und Artikel</p>
            </div>
          )}
        </div>
        ...
        <div 
          ...
          style={methodContainerStyle(paymentMethod === 'PAYPAL')}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {paymentMethod === 'PAYPAL' && (
            <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#f0f9ff', borderRadius: '10px', border: '1px dashed #0070ba', fontSize: '16px', lineHeight: '1.6' }}>
              ...
              <strong>berndos59@gmail.com</strong><br/>
              <p style={{ marginTop: '10px', fontSize: '14px', fontStyle: 'italic' }}>Bitte manuelle Überweisung mit Verwendungszweck: Name und Artikel</p>
            </div>
          )}
        </div>
      ...
      <div style={{ backgroundColor: '#fff', padding: '25px', borderRadius: '20px', border: '1px solid #eee', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <button 
            ...
            style={{...btnStyle, opacity: (isSubmitting || cartItems.length === 0) ? 0.6 : 1}}
          >
...
const btnStyle = { backgroundColor: '#8b5cf6', color: 'white', padding: '20px', borderRadius: '15px', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: '20px', marginTop: '15px', boxShadow: '0 4px 6px rgba(139, 92, 246, 0.3)' };
const methodContainerStyle = (active: boolean) => ({
  padding: '20px',
  borderRadius: '15px',
  border: active ? '2px solid #8b5cf6' : '1px solid #eee',
