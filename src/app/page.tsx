      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(160deg, #fef9ef 0%, #f5f0ff 55%, #fdf4ff 100%)', paddingTop: 'clamp(3rem, 10vw, 6rem)', paddingBottom: 'clamp(5rem, 12vw, 8rem)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '1100px', margin: '0 auto', padding: '0 1.25rem', textAlign: 'center' }}>
          <h1 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 'clamp(2.25rem, 8vw, 5rem)', fontWeight: 900, color: '#1c1917', letterSpacing: '-0.02em', lineHeight: 1.05, marginBottom: '1.375rem' }}>
            Berndos{' '}
            <span style={{ background: 'linear-gradient(135deg, #6d28d9, #a855f7, #ec4899)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', fontStyle: 'italic' }}>
              Flohmarkt
            </span>
          </h1>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 'clamp(1.5rem, 4vw, 3rem)', flexWrap: 'wrap' }}>
            {[{ icon: '🏷️', label: 'Einzelstuecke', sub: 'Jedes einmalig' }, { icon: '💳', label: 'Sicher zahlen', sub: 'PayPal & Ueberweisung' }, { icon: '📦', label: 'Schneller Versand', sub: 'Per DHL' }].map((item) => (
