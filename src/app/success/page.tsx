...
export default function SuccessPage() {
  ...
  const steps = [
    ...
    { icon: '💳', text: 'Bitte überweise den Betrag mit deinem Namen als Verwendungszweck' },
    { icon: '📦', text: 'Nach Zahlungseingang versenden wir deinen Artikel' },
    { icon: '🚚', text: 'Du erhältst die Sendungsverfolgung per E-Mail' },
  ]
  ...
  return (
    <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'linear-gradient(135deg, #f5f3ff 0%, #fdf2f8 50%, #faf5ff 100%)' }}>
      <div style={{ textAlign: 'center', maxWidth: '520px', opacity: show ? 1 : 0, transform: show ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
        ...
        <div style={{ width: '100px', height: '100px', background: 'linear-gradient(135deg, #7c3aed, #ec4899)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 2rem', fontSize: '3rem', boxShadow: '0 20px 40px rgba(124, 58, 237, 0.3)', color: 'white', fontWeight: 900 }}>
          ✓
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '1.5rem', marginBottom: '2rem', border: '1px solid #e9d5ff', boxShadow: '0 4px 20px rgba(124, 58, 237, 0.08)', textAlign: 'left' }}>
          <h3 style={{ color: '#7c3aed', fontWeight: 700, marginBottom: '1rem', fontSize: '1rem', margin: '0 0 1rem' }}>
            Naechste Schritte:
          </h3>
